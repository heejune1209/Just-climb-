...
public class UI_Inventory : UI_Scene
{
    ...
    {
        // UI_Inventory 팝업이 활성화될 때
        // 현재 아이템 개수와 쿨타임을 즉시 반영
        for (int i = 0; i < _itemIds.Length; i++)
        {
            string id = _itemIds[i];  // 슬롯 인덱스에 맞는 아이템 ID

            // 슬롯 개수 텍스트를 ItemManager로부터 가져온 수량으로 설정
    ...
    {
        // UI_Inventory가 비활성화될 때 이벤트 구독 해제
