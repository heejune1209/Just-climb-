...
graph LR
  ...
  subgraph UI Layer
    ...
    UIBase[UI_Base]
    UIPopup[UI_Popup]
    UISceneClass[UI_Scene]

    subgraph Title Scene
      direction TB
      UIMain[UI_Main]
      UIAch[UI_Achievement]
      UISet[UI_Settings]
      UIChar[SelectCharacter]
    end
    ...
    subgraph Lobby Scene
      ...
      UIShop[UI_Shop]
      UIWarn[UI_Warning]
      UISelectChap[UI_SelectChapter]
      UISelectStage[UI_SelectStage]
      UIGenc[UI_GenericInfoPopup]
      UIWorld[UI_WorldView]
      UIRank[UI_Ranking]
    end

    subgraph Stage Scene
      direction TB
      UIStage[UI_Stage]
      UIInfo[UI_Information]
      UIRes[UI_Result]
      UIInv[UI_Inventory]
      UITut[UI_Tutorial]
      UIWarnS[UI_Warning]
  ...
  %% Managers → 각 매니저 Init 호출
  Managers -->|Init() 호출| DM
  Managers -->|Init() 호출| Curr
  Managers -->|Init() 호출| ItemM
  Managers -->|Init() 호출| StageM
  Managers -->|Init() 호출| RM
  Managers -->|Init() 호출| UIM
  Managers -->|Init() 호출| SMEX
  Managers -->|Init() 호출| SO
  Managers -->|Init() 호출| Pool
  Managers -->|Init() 호출| GM
  Managers -->|Init() 호출| Def
  Managers -->|Init() 호출| Util
  Managers -->|Init() 호출| Ext
  ...
  UILobby --> UIShop
  UILobby --> UIWarn
  UILobby --> UISelectChap
  UILobby --> UISelectStage
  UILobby --> UIGenc
  UILobby --> UIWorld
  UILobby --> UIRank
